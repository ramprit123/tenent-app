...
export class NotificationService {
  ...
  async findOne(id: string) {
    const rec = await this.prisma.notification.findUnique({ where: { id } });
    if (!rec) throw new NotFoundException('Notification not found');
    return rec;
  }

  async create(data: any, ctx: { tenantId?: string; userId?: string } = {}) {
    const rec = await this.prisma.notification.create({ data });
    // audit log
  ...
  async update(
    ...
    const rec = await this.prisma.notification.update({
      where: { id },
      data,
    });
    await this.audit.log({
  ...
  async remove(id: string, ctx: { tenantId?: string; userId?: string } = {}) {
    await this.findOne(id);
    const rec = await this.prisma.notification.delete({ where: { id } });
    await this.audit.log({
      tenantId: ctx.tenantId,
      userId: ctx.userId,
      service: 'notification',

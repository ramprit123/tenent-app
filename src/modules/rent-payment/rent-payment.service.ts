...
export class RentPaymentService {
  ...
  async findOne(id: string) {
    const rec = await this.prisma.rentPayment.findUnique({ where: { id } });
    if (!rec) throw new NotFoundException('RentPayment not found');
    return rec;
  }

  async create(data: any, ctx: { tenantId?: string; userId?: string } = {}) {
    const rec = await this.prisma.rentPayment.create({ data });
    // audit log
  ...
  async update(id: string, data: any, ctx: { tenantId?: string; userId?: string } = {}) {
    ...
    const rec = await this.prisma.rentPayment.update({
      where: { id },
      data,
    });
    await this.audit.log({
  ...
  async remove(id: string, ctx: { tenantId?: string; userId?: string } = {}) {
    await this.findOne(id);
    const rec = await this.prisma.rentPayment.delete({ where: { id } });
    await this.audit.log({
      tenantId: ctx.tenantId,
      userId: ctx.userId,
      service: 'rent-payment',

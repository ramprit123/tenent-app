...
@Controller('kyc-record')
export class KycRecordController {
  constructor(private readonly service: KycRecordService) {}
  ...
  @Post()
  async create(@Body() dto: CreateKycRecordDto, @Req() req: any) {
    const ctx = { tenantId: req?.tenantId, userId: req?.user?.sub || req?.user?.id };
    return this.service.create(dto.data || dto, ctx);
  }

  @Put(':id')
  async update(@Param('id') id: string, @Body() dto: UpdateKycRecordDto, @Req() req: any) {
    const ctx = { tenantId: req?.tenantId, userId: req?.user?.sub || req?.user?.id };

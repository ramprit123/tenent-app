...
export class KycRecordService {
  ...
  async findOne(id: string) {
    const rec = await this.prisma.kycRecord.findUnique({ where: { id } });
    if (!rec) throw new NotFoundException('KycRecord not found');
    return rec;
  }

  async create(data: any, ctx: { tenantId?: string; userId?: string } = {}) {
    const rec = await this.prisma.kycRecord.create({ data });
    // audit log
  ...
  async update(id: string, data: any, ctx: { tenantId?: string; userId?: string } = {}) {
    ...
    const rec = await this.prisma.kycRecord.update({
      where: { id },
      data,
    });
    await this.audit.log({
  ...
  async remove(id: string, ctx: { tenantId?: string; userId?: string } = {}) {
    await this.findOne(id);
    const rec = await this.prisma.kycRecord.delete({ where: { id } });
    await this.audit.log({
      tenantId: ctx.tenantId,
      userId: ctx.userId,
      service: 'kyc-record',

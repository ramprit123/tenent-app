...
export class AuditLogService {
  ...
  async findOne(id: string) {
    ...
    if (!rec) throw new NotFoundException('AuditLog not found');
    return rec;
  }

  async create(data: any, ctx: { tenantId?: string; userId?: string } = {}) {
    const rec = await this.prisma.auditLog.create({ data });
    // audit log
  ...
  async update(id: string, data: any, ctx: { tenantId?: string; userId?: string } = {}) {
    ...
    const rec = await this.prisma.auditLog.update({
      where: { id },
      data,
    });
    await this.audit.log({
  ...
  async remove(id: string, ctx: { tenantId?: string; userId?: string } = {}) {
    await this.findOne(id);
    const rec = await this.prisma.auditLog.delete({ where: { id } });
    await this.audit.log({
      tenantId: ctx.tenantId,
      userId: ctx.userId,
      service: 'audit-log',
